...
public class PrGameSetup : MonoBehaviour {
    ...
    void StartGame()
    {
        ...
        if (PrPlayerInfo.player1 && PrPlayerInfo.player1.loadPrevSettings == true)
        {
            ...
            playersControllers[0].transform.position = PrPlayerInfo.player1.lastPlayerPosition;
            if (useLives)
                livesPerPlayer[0] = PrPlayerInfo.player1.lives;
            if (actualGameOverUI)
            {
   ...
   /* void UpdateSurvivalTimer()
    ...
    {
        if (GameOverUI && actualGameOverUI)
        ...
        CallEnemiesToStop();
        foreach (PrTopDownCharController p in playersControllers)
        {
            if (p != null)
            {
                ...
                if (objectivesManager)
                    PrPlayerInfo.player1.lastObjectiveActive = objectivesManager.actualObjective;
                if (useLives)
                    PrPlayerInfo.player1.lives = livesPerPlayer[0] -1;
